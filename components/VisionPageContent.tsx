...
import { Eye, Flag, Sparkles, TrendingUp } from 'lucide-react';
import { vision } from '@/lib/aboutContent';

const highlightIcons = [TrendingUp, Sparkles, Flag, Eye];
  ...
  return (
    <div className="w-full min-w-0 bg-slate-50 pb-16">
      ...
      <div className="w-full max-w-5xl mx-auto px-4 sm:px-6 md:px-8 pt-8 md:pt-10 space-y-8 md:space-y-10">
        <div className="w-full rounded-3xl border border-amber-200 bg-linear-to-br from-white via-amber-50/50 to-white p-6 sm:p-8 md:p-10 shadow-lg">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4">
          {vision.highlights.map((item, index) => {
            ...
            return (
              <div
                key={item}
                className="flex items-center gap-3 rounded-2xl border border-slate-100 bg-white p-4 md:p-5 shadow-sm"
              >
                <div className="shrink-0 h-10 w-10 rounded-xl bg-secondary/15 text-secondary flex items-center justify-center">
