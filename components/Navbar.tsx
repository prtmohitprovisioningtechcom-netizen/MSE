  ...
  return (
    <header className="sticky top-0 z-50 w-full transition-all duration-300">
      ...
      <nav className="glass-nav shadow-sm px-4 md:px-8 py-3 flex items-center justify-between">
          ...
          {navGroups.map((link) => (
            <div key={link.name} className="relative group">
              <a
                ...
                className={`px-3 py-2 rounded-md text-xs font-semibold tracking-wide uppercase transition-all duration-100 flex items-center gap-1 cursor-pointer ${
                ...
                <div className="absolute left-0 top-full pt-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all">
                  <div className="w-72 rounded-2xl bg-white border border-slate-100 shadow-xl p-2">
                    {link.children.map((child) => (
                      <a key={`${link.name}-${child.name}`} href={child.path} onClick={(e) => handleNav(e, child.path)} className="block rounded-xl px-3 py-2 text-xs font-semibold text-slate-600 hover:bg-primary/5 hover:text-primary cursor-pointer active:scale-95 active:text-secondary transition-all duration-100">
                        {child.name}
                      </a>
                    ))}
                  </div>
        ...
        <a href="/membership" onClick={(e) => handleNav(e, '/membership')} className="hidden xl:inline-flex px-4 py-2.5 bg-secondary hover:bg-secondary-hover text-white text-xs font-extrabold rounded-xl shadow-sm uppercase tracking-wider transition-all cursor-pointer active:scale-95">
          Apply Now
        </a>
              ...
              <div key={link.name}>
                ...
                {link.children && (
                  <div className="ml-4 mt-1 space-y-1 border-l border-slate-100 pl-3">
                    {link.children.map((child) => (
                      <a key={`${link.name}-${child.name}`} href={child.path} onClick={(e) => handleNav(e, child.path)} className="block py-1.5 text-xs font-semibold text-slate-500 cursor-pointer active:text-secondary transition-all duration-100">
                        {child.name}
                      </a>
                    ))}
                  </div>
